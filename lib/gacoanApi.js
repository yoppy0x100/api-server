...
class gacoanApi {
  ...
  async run() {
    ...
    const workers = Array.from({ length: Math.min(this.#CONCURRENCY, this.#branchCode.length) }, () => this.worker());
  ...
  async processUrl(dataBranch) {
    let bearer = this.generateAuthKey("POST", this.#config.baseUrl + "qsv1/setting/branch", "", this.#config.baseKey);
    const response = await fetch(this.#config.baseUrl + "qsv1/setting/branch", {
      method: "POST",
      headers: {
